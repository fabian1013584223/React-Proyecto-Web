...
const ShowProductos = () => {
const iD='9FFFC55A-186F-4D62-88B4-78B43B2948C1'
  const url = `https://localhost:7289/api/stocks/${iD}/productos`;
  const [productos, setProductos] = useState([]);
  const [pId, setPId] = useState('');
  const URL=`https://localhost:7289/api/productos`;
...
console.log(pId)
  ...
  const openModal = (op, pId, nombre, cantidad, precio, estado, lugar) => {
    setPId('');
    setNombre('');
    setCantidad('');
    setPrecio('');
  ...
  const deleteProducto = (pId, nombre) => {
                  ...
                  {productos.map((producto, i) => (
                    <tr key={producto.pId}>
                      ...
                      <td>
                        <button
                          onClick={() =>
                            openModal(2, producto.pId, producto.nombre, producto.cantidad, producto.precio, producto.estado, producto.lugar)
                          }
                        ...
                        &nbsp;
                        <button onClick={() => deleteProducto(producto.pId, producto.nombre)} className="btn btn-danger">
