...
const ShowProveedor = () => {
  const url = 'https://localhost:7289/api/proveedores';
  const [proveedor, setProveedor] = useState([]);
  const [ProveedorId, setProveedorId] = useState('');
  const URL = `https://localhost:7289/api/proveedores/${ProveedorId}`;
  ...
  const openModal = (op, ProveedorId, nombre, apellido,  correo,  contacto, direccion) => {
    setProveedorId('');
    setNombre('');
    setApellido('');
    setCorreo('');
  ...
  const validar = () => {
    ...
    var metodo;
    if (nombre.trim() === '') {
      show_alerta('Escriba nombre de Proveedor', 'warning');
    
    } else if (apellido.trim() === '') {
      show_alerta('Escriba apellido de Proveedor', 'warning');
    } else if (correo.trim() === '') {
      show_alerta('Escriba correo de Proveedor', 'warning');
    } else if (contacto === 0) {
      show_alerta('Escriba contacto de Proveedor', 'warning');
    } 
    else if (direccion.trim() === '') {
        show_alerta('Escriba la direccion del Proveedor', 'warning');
  ...
  const deleteUsuario = (ProveedorId, name) => {
    const MySwal = withReactContent(Swal);

    MySwal.fire({
      title: `¿Seguro desea eliminar este usuario ${nombre}?`,
      icon: 'question',
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.delete(`${url}/${ProveedorId}`);
  ...
  return (
    <div className="App">
        ...
        <div className='row mt-3'>
          <div className='col-md-4 offset-4'>
            <div className='d-grid mx-auto'>
              <button onClick={() => openModal(1)} className='btn btn-dark' data-bs-toggle='modal' data-bs-target='#modalproveedor'>
                  ...
                  {proveedor.map((proveedor, i) => (
                    <tr key={proveedor.ProveedorId}>
                      ...
                      <td>
                        <button
                          onClick={() =>
                            openModal(2, proveedor.ProveedorId, proveedor.nombre, proveedor.apellido, proveedor.correo, proveedor.contacto,proveedor.direccion)
                          }
                        ...
                        <button onClick={()=>deleteUsuario(proveedor.ProveedorId,proveedor.nombre)} className="btn btn-danger">
      ...
      <div id='modalproveedor' className='modal fade' aria-hidden='true'>
