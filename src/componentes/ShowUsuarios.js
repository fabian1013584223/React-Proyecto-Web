...
const ShowUsuarios = () => {
  ...
  const URL = `https://localhost:7289/api/usuarios/${usuarioid}`;
  ...
  const openModal = (op, usuarioid, nombre, direccion, contraseña, apellido, correo, contacto, pais) => {
    setusuarioid('');
    setNombre('');
    setDireccion('');
    setContraseña('');
  ...
  const validar = () => {
    ...
    var metodo;
    if (nombre.trim() === '') {
      show_alerta('Escriba nombre de usuario', 'warning');
    } else if (direccion.trim() === '') {
      show_alerta('Escriba dirección y país del usuario', 'warning');
    } else if (contraseña.trim() === '') {
      show_alerta('Escriba contraseña válida', 'warning');
    } else if (apellido.trim() === '') {
      show_alerta('Escriba apellido de usuario', 'warning');
    } else if (correo.trim() === '') {
      show_alerta('Escriba correo de usuario', 'warning');
    } else if (contacto === '') {
      show_alerta('Escriba contacto de usuario', 'warning');
    } else {
      if (operation === 1) {
        parametros = {
          nombre: nombre.trim(),
  ...
  const deleteUsuario = (usuarioid, nombre) => {
        ...
        <div className='row mt-3'>
          <div className='col-12 col-lg-8 offset-0 offset-lg-12'>
            <div className='table-responsive'>
              <table className='table table-bordered'style={{width:'100px'}}>
                <thead>
                  <tr>
                    <th>#</th>
                    <th>USUARIO</th>
                  ...
                  {usuarios.map((usuarios, i) => (
                    <tr key={usuarios.usuarioid}>
                      ...
                      <td>
                        <button
                          onClick={() =>
                            openModal(2, usuarios.usuarioid, usuarios.nombre, usuarios.direccion, usuarios.contraseña, usuarios.apellido, usuarios.correo, usuarios.contacto,usuarios.pais)
                          }
                        ...
                        <button onClick={()=>deleteUsuario(usuarios.usuarioid,usuarios.nombre)} className="btn btn-danger">
