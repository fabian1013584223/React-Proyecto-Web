...
const ShowCliente = () => {
const iD='c9d4c053-49b6-410c-bc78-2d54a9991870'
  const url = `https://localhost:7289/api/usuarios/${iD}/clientes`;
  const [Cliente, setClientes] = useState([]);
  const [ClienteId, setClienteId] = useState('');
  const URL=`https://localhost:7289/api/usuarios`;
  ...
  const openModal = (op, ClienteId, numerodocumento, tipodocumento, nombre, apellido, correo, contacto) => {
    setClienteId('');
    setNumerodocumento('');
    setTipodocumento('');
    setNombre('');
  ...
  const validar = () => {
    ...
    if (numerodocumento.trim() === '') {
      ...
      show_alerta('Escriba la cantidad del producto', 'warning');
    } else if (nombre.trim() === '') {
      show_alerta('Escriba el precio del producto', 'warning');
    } else if (apellido.trim() === '') {
      show_alerta('Escriba el estado del producto', 'warning');
    } else if (correo.trim() === '') {
      show_alerta('Escriba el lugar del producto', 'warning');
  ...
  const deleteCliente = (ClienteId, nombre) => {
    ...
    MySwal.fire({
      title: `¿Seguro desea eliminar este producto ${nombre}?`,
      icon: 'question',
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${URL}/${ClienteId}`);
        ...
        <div className='row mt-3'>
          <div className='col-md-4 offset-4'>
            <div className='d-grid mx-auto'>
              <button onClick={() => openModal(1)} className='btn btn-dark' data-bs-toggle='modal' data-bs-target='#modalCliente'>
                  ...
                  {Cliente.map((Cliente, i) => (
                    <tr key={Cliente.ClienteId}>
                      ...
                      <td>
                        <button
                          onClick={() =>
                            openModal(2, Cliente.ClienteId, Cliente.numerodocumento,Cliente.tipodocumento,Cliente.nombre, Cliente.apellido, Cliente.correo, Cliente.contacto)
                          }
                        ...
                        &nbsp;
                        <button onClick={() => deleteCliente(Cliente.ClienteId, Cliente.numerodocumento)} className="btn btn-danger">
