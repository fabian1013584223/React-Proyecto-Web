...
const ShowFactura = () => {
  const url = 'https://localhost:5001/api/Factura';
  const [factura, setFactura] = useState([]);
  const [id, setId] = useState('');
  const [fecha, setFecha] = useState('');
  const URL = `https://localhost:5001/api/proveedor/{companyId}/factura`; 
  ...
  const deleteUsuario = (id,fecha) => {
    const MySwal = withReactContent(Swal);

    MySwal.fire({
      title: `¿Seguro desea eliminar esta factura ${fecha}?`,
      icon: 'question',
        ...
        <div className='row mt-3'>
          <div className='col-md-4 offset-4'>
            <div className='d-grid mx-auto'>
              <button onClick={() => openModal(1)} className='btn btn-dark' data-bs-toggle='modal' data-bs-target='#modalFactura'>
    ...
    <tbody className='table-group-divider '>
      {factura.map((factura, i) => (
        <tr key={factura.id}>
          ...
          <td>
            <button
              onClick={() => openModal(2, factura.id, factura.fecha)}
            ...
            <button
              onClick={() => deleteUsuario(factura.id, factura.fecha)}
      ...
      <div id='modalFactura' className='modal fade' aria-hidden='true'>
