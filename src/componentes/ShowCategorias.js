...
const ShowCategorias = () => {
  const url = 'https://localhost:7289/api/categorias';
  const [categorias, setCategorias] = useState([]);
  const [categoriaId, setcategoriaId] = useState('');
  const URL = `https://localhost:7289/api/categorias/${categoriaId}`; 
  ...
  const openModal = (op, categoriaId, nombre) => {
    setcategoriaId('');
    setNombre('');
    setOperation(op);
    if (op === 1) {
  ...
  const validar = () => {
    ...
    var metodo;
    if (nombre.trim() === '') {
      show_alerta('Escriba el nombre de la categoría', 'warning');
    } else {
      if (operation === 1) {
        parametros = {
          Nombre: nombre.trim(),
  ...
  const deleteCategoria = (categoriaId, nombre) => {
    const MySwal = withReactContent(Swal);

    MySwal.fire({
      title: `¿Seguro desea eliminar esta categoría: ${nombre}?`,
      icon: 'question',
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${url}/${categoriaId}`);

          show_alerta('Categoría eliminada exitosamente', 'success');
          getCategorias();
        } catch (error) {
                ...
                <tbody className='table-group-divider'>
                  {categorias.map((categoria, i) => (
                    <tr key={categoria.categoriaId}>
                      ...
                      <td>
                        <button
                          onClick={() => openModal(2, categoria.categoriaId, categoria.nombre)}
                        ...
                        &nbsp;
                        <button onClick={() => deleteCategoria(categoria.categoriaId, categoria.Nombre)} className="btn btn-danger">
